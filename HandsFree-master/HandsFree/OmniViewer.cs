...
namespace HandsFree {
    class OmniViewer {
    	...
    	public static void Show(ref Image<Gray, byte> image, string name) {
            if (!Suppressed)
                if (viewerDict.ContainsKey(name)) viewerDict[name].Image = image.Resize(2d, Emgu.CV.CvEnum.INTER.CV_INTER_LINEAR);
                else { viewerDict.Add(name, new ImageViewer(image.Resize(2d, Emgu.CV.CvEnum.INTER.CV_INTER_LINEAR))); viewerDict[name].Show(); }
    	}
        ...
        public static void CloseAll() {
            Suppressed = true;
            foreach (ImageViewer viewer in viewerDict.Values) viewer.Hide();
