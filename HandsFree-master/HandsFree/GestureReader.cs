...
namespace HandsFree {
    class GestureReader {
        ...
        private static bool checkBounds(ref int valMin, ref int valMax, int upperBound) { //check if coordinate values are out of bounds, and set values to be within bounds (two birds one function)
            if (valMin < 0) { valMin = 0; return false; }
            else if (valMax >= upperBound) { valMax = upperBound - 1; return false; }
            return true;
        }


        private static Point getCircumferencePoint(Point origin, int radius, double angle) {    //get end point of a line given the origin of the line, angle between the line and the horizontal line drawn across origin, and length of the line
        ...
        private static bool bresenham(ref int x, ref int y, ref int x2, ref int y2, ref Image<Gray, byte> imageSource, out Point ptResult) {    //bresenhanm's line draing algorithm
            int w = x2 - x;
            int h = y2 - y;
            ...
            for (int i = 0; i <= longest; i++) {
                if (checkBounds(ref x, ref x, imageWidthCropped) && checkBounds(ref y, ref y, imageHeightCropped)) {
                    if (imageSource.Data[y, x, 0] == 255 || imageSource.Data[y, x, 0] == 64)    //only keep drawing if the current pixel the line is drawing across is a white one or has the value of 64
                        imageSource.Data[y, x, 0] = 64;
                    else { ptResult.X = x; ptResult.Y = y; return true; }    //otherwise stop and return the position of the last point the line travels across
                }
                else return false;
